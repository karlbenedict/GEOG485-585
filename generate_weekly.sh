...
find . -name "week*.md" -exec /usr/local/bin/pandoc --template default.latex --pdf-engine=xelatex  -o '{}'.pdf weekXX.md '{}' \;
...
/usr/local/bin/pandoc --toc  --top-level-division=chapter --template default.latex --self-contained --standalone week*.md -o week_all.pdf --pdf-engine=xelatex
